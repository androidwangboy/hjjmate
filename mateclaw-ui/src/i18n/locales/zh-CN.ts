export default {
  ...
  plans: {
    title: '计划看板',
    subtitle: '把专家的计划与步骤按状态铺成看板',
    selectAgent: '选择专家',
    allEmployees: '全部专家',
    unknownAgent: '未知专家',
    noPlansAll: '团队暂无任何计划。让计划执行型专家跑一个多步任务后，这里就会出现。',
    detail: '计划详情',
    assignee: '受派专家',
    progress: '进度',
    created: '创建',
  ...
  settings: {
    ...
    migration: {
      title: '数据迁移（导出 / 导入）',
      desc: '把专家、团队、技能、知识库、工作流、触发器打包成环境无关的 .mcbundle，在另一个环境导入。导入只新增，绝不覆盖目标环境已有数据。',
      safetyTitle: '非破坏性保证',
      safety1: '导入只新增行，永不修改、永不删除目标环境已有的数据',
      safety2: '不提供覆盖语义：同名且内容不同时，以「xxx (imported)」新建共存',
      safety3: '每次导入生成一个批次号，可一键撤销（只删本批新建的行）',
      exportTitle: '导出',
      exportHint: '勾选要导出的模块；可只导出某位专家（会自动带上它依赖的技能与知识库）。',
      modAgents: '数字专家',
      modTeams: '团队',
      modSkills: '技能',
      modKbs: '知识库',
      ...
      modTriggers: '触发器',
      pickAgents: '只导出指定专家',
      pickAgentsHint: '留空表示导出全部；选择后会连同其依赖一并打包。',
      allAgents: '全部专家',
      includeKb: '包含知识库原料（导入后重新生成 Wiki 页面）',
      includePages: '包含已生成的 Wiki 页面（包体会变大，导入后无需等待重新生成）',
      includeMemory: '包含专家记忆文件（AGENTS / MEMORY / PROFILE / KNOWLEDGE，不含 SOUL）',
      importScopeHint: '导入范围由导出包决定：包里含页面就恢复页面，含记忆就写入记忆；已存在的专家记忆不会被改动。',
      exportBtn: '生成导出包',
      exportOk: '导出包已生成',
      exportFail: '导出失败',
      importTitle: '导入',
      importHint: '先预览再提交。预览不会写入任何数据。',
      conflictPolicy: '同名冲突策略',
      conflictPolicyHint: '目标环境已存在同名但内容不同时的处理方式。',
      policyRename: 'rename：改名导入（推荐，保证导入成功）',
      policySkip: 'skip：跳过，保留目标环境现有版本',
      policyFail: 'fail：中止整个导入',
      pickFile: '点击选择或拖拽 .mcbundle 文件到此处',
      previewBtn: '预览（不写入）',
      previewFail: '预览失败',
      importBtn: '提交导入',
      importOk: '导入完成',
      ...
      batchId: '本次批次号',
      revertTitle: '撤销某次导入',
      revertHint: '只删除该批次新建的行，目标环境原有数据不受影响。',
      revertBtn: '撤销导入',
      batchPlaceholder: '输入批次号，如 a1b2c3d4',
      revertConfirm: '确认撤销批次 {batch} 的导入？只会删除该批次新建的数据。',
      revertOk: '已撤销，删除 {n} 条数据',
      revertFail: '撤销失败',
    },
    models: {
      sidecar: {
    ...
    model: {
      ...
      claudeCodeOauthRevokeHint: '请在 Claude Code 客户端中退出登录。HjjMate 不会修改 Claude Code 的本地凭据。',
    ...
    localTools: {
      desktopOnly: '本地工具仅在桌面客户端内可用。请在 HjjMate 桌面应用中打开此页面进行管理。',
      enableLabel: '启用本地工具',
  ...
  agents: {
    ...
    api: {
      open: 'API 管理',
      kicker: '对外调用',
      summary: '把这位专家发布给 CRM、门户或其他业务系统。',
      published: '已发布',
      unpublished: '未发布',
      keys: '个 Key',
      callsToday: '次今日调用',
      loading: '正在读取 API 状态…',
      openConsole: '打开 API 控制台',
      hint: '控制台可管理 Key、限流、文档、调用日志、统计和在线测试。',
      publishSuccess: 'API 已发布',
      unpublishSuccess: 'API 已取消发布',
      updateFailed: 'API 状态更新失败',
    },
    columns: {
      name: '名称',
      description: '描述',
    ...
    fields: {
      ...
      runtimeNativeHint: '使用 HjjMate 原生 Agent Loop。',
      ...
      workspaceBasePathHint: '可选。为该专家指定独立的工作目录：相对路径会基于工作区根目录解析，也可填写绝对路径。留空则继承工作区默认目录。',
      modelName: '模型',
      modelGlobalDefault: '使用全局默认模型',
      modelHint: '为该专家单独指定模型，留空则跟随「设置 → 模型」中的全局默认。',
      tags: '标签',
    ...
    guide: {
      ...
      open: '管理行为指南',
      desc: '这里是该专家的行为手册（AGENTS.md）：规定何时写记忆、各文件放什么、可用的记忆工具。上方的岗位／目标／背景是专家的出厂身份；PROFILE／MEMORY 等可演化的记忆在「记忆」菜单管理。',
      empty: '该专家还没有行为指南（AGENTS.md）。',
      create: '创建行为指南',
      saved: '已保存',
      addSection: '新增小节',
      addSectionPlaceholder: '小节标题',
      scaffold: '## 何时写记忆\n\n（说明这个专家在什么情况下应当记录到 MEMORY.md / PROFILE.md。）\n\n## 文件职责\n\n（说明 SOUL.md / PROFILE.md / MEMORY.md 各自该放什么内容。）\n',
    },
    thinkingLevels: {
      auto: '跟随模型',
      off: '关闭',
    ...
    messages: {
      noDescription: '暂无描述',
      noTagline: '还没填岗位与目标',
      deleteConfirm: '确认让该专家离职吗？删除后不可恢复。',
      loadFailed: '加载专家列表失败',
      saveFailed: '保存专家失败',
      saveSuccess: '专家已保存',
      deleteFailed: '删除专家失败',
    ...
    wizard: {
      ...
      kicker: 'AI 创建',
      title: '一句话创建专家',
      subtitle: '描述你需要的专家，AI 自动配好人设、类型与工具，确认即可上岗',
      ...
      successSubtitle: '新专家已加入你的团队，随时可以开始对话',
  ...
  acp: {
    ...
    env: {
      ...
      hints: {
        ...
        dashscope: '阿里云 DashScope key — 跟 HjjMate .env 里的 DASHSCOPE_API_KEY 同一个值',
  ...
  datasources: {
    ...
    placeholders: {
      ...
      port: '3306',
      databaseName: '数据库名',
      serviceName: '服务名 Service Name，如 ORCL',
  ...
  live: {
    ...
    headline: {
      loading: '正在看看...',
      allQuiet: '一片安静。当前没有专家在工作。',
      working: '你的 {n} 个专家正在干活。',
    ...
    confirm: {
      ...
      endBody: '这会立即终止该专家，正在进行的工作会丢失。用户会看到「已被管理员停止」。',
      subTitle: '停下这个帮手？',
      subBody: '停止 {name}。负责的专家会继续运行。',
      sweepTitle: '清理卡住的专家？',
      sweepBody: '有 {n} 个专家看起来卡住了，全部停掉吗？',
    },
    toast: {
      stopped: '已要求停止。',
      ended: '已停止。',
  ...
  wiki: {
    ...
    adv: {
      ...
      perm: {
        tab: '权限 & 审批',
        title: '专家页面类型权限',
        desc: '为指定专家配置在本知识库下按 pageType 的读/创建/更新/删除权限；写操作可设为需审批。无规则时默认放行，一旦存在规则则未覆盖的类型按失败安全（拒绝）处理。',
        agent: '专家',
        selectAgent: '选择一位专家…',
        pageType: '页面类型',
        policy: '写策略',
        noRows: '该专家暂无权限规则（默认放行）。',
  ...
  workflows: {
    ...
    canvas: {
      ...
      fields: {
        ...
        agentPlaceholder: '选择数字专家',
        agentMissing: '当前数字专家不在列表中：{name}',
    ...
    dialogs: {
      createTitle: '新建工作流',
      fieldName: '名称',
      namePlaceholder: '例如：expert-onboarding',
      fieldDescription: '描述',
    ...
    generate: {
      ...
      placeholder: '例如：每周一早上 9 点让销售专家汇总上周数据，老板确认后发到飞书运营群',
      submit: '生成草稿',
      running: '生成中...',
      retry: '重试',
      accept: '采用并打开编辑器',
  ...
  login: {
    ...
    heroDesc: 'HjjMate 的重点不是把更多能力堆进聊天框，而是把上下文、知识、执行和记忆收拢成一个可靠的工作面。',
  ...
  channels: {
    ...
    empty: {
      title: '连接第一个渠道',
      desc: '把你的数字专家接入团队已经在用的 IM 工具——Slack、企业微信、Telegram 等。',
      cta: '连接一个渠道',
    },
    stats: {
      active: '{n} 个运行中',
    ...
    webchatHint: 'Web / API 接入既可把 HjjMate 聊天挂件嵌入外部网站，也可作为纯后端 SSE 接口直接调用。配置 API Key 后，前端可引入 WebChat SDK；后端可直接 POST /api/v1/channels/webchat/stream，头带 X-MC-Key、体传 {message, visitorId, 可选 sessionId/agentId}。',
