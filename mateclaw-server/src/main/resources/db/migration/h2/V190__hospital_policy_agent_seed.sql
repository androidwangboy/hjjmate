-- V190: Hospital Policy / Regulation Q&A agent seed.
-- A single React-type expert that answers questions about the hospital's
-- internal policies (HR, admin, medical, nursing, infection control,
-- emergency, infosec, etc.). Designed to be paired with a Wiki KB that the
-- hospital fills with its own regulation documents — the agent does RAG
-- retrieval under the hood and is system-prompted to refuse to fabricate.
--
-- Idempotent upsert on id (H2 MERGE). Companion files in mysql/ and
-- kingbase/ use the dialect-equivalent INSERT ... ON DUPLICATE/ON CONFLICT.
-- Also seeded in db/data-zh.sql and db/data-en.sql for fresh installs.
--
-- id 1000000100 was deliberately picked outside the existing built-in
-- range (1000000001 ~ 1000000640) to avoid collisions with the four
-- shipped agents and the content-studio tool cluster.
