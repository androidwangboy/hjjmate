...
description: HjjMate Team Run 用一个 runId 串起 Lead、任务 DAG、成员执行、最终汇总与交付物，并在 Chat、Agents、Teams 三个页面提供统一视图。
head:
  - - meta
    - name: keywords
      content: Agent团队,任务板,看板,多Agent协作,派发,交付物,团队协作,HjjMate
---

# Team Run 与团队协作（2.1.0+）

> **以前是"一个专家带子任务"。现在是"一个团队围着一块任务板"。**

子专家委派（`delegateToAgent`）解决的是"一个人临时叫帮手"：同步等结果、一对一、过程黑盒。但真实的复杂交付不长这样——它长得像一个项目：**拆任务、标依赖、并行推进、卡点审批、交付物归档、随时能看谁在干什么**。

团队协作把这套项目机制搬进 HjjMate：你建一个**团队**，指定一个 **Lead** 专家、若干**成员**专家；对 Lead 说一句目标，它把目标拆成任务落到**共享任务板**上；派发引擎把任务自动分给成员**并行执行**；成员完成后结果自动通报回 Lead，由它汇总、补派、直到整件事干完。你全程在 Teams 页旁观——或者直接往板上投任务。
...
| 概念 | 说明 |
|------|------|
| **团队（Team）** | 一组专家 + 一块任务板。一个专家可以加入多个团队。 |
...
**团队上下文注入**：加入团队的专家，system prompt 会注入团队名册与协作行为准则；Lead 每轮对话还会**动态注入实时看板快照**——它不需要先调 `list` 才知道板上有什么，多轮对话也不会因为"忘了看板"重复建任务。
