...
团队协作把这套项目机制搬进 MateClaw：你建一个**团队**，指定一个 **Lead** 专家、若干**成员**专家；对 Lead 说一句目标，它把目标拆成任务落到**共享任务板**上；派发引擎把任务自动分给成员**并行执行**；成员完成后结果自动通报回 Lead，由它汇总、补派、直到整件事干完。你全程在 Teams 页旁观——或者直接往板上投任务。
