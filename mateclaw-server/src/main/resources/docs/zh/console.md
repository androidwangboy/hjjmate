...
控制台是每个 HjjMate 部署都自带的 Vue 3 SPA。它跑在你的浏览器里（或者 Electron 桌面窗口里），通过 REST + SSE 跟 Spring Boot 后端对话，把 HjjMate 的每一项能力——聊天、Agent、知识、工具、技能、渠道、安全、定时任务、用量分析——都放在**同一套登录**后面。
...
- **卡死专家**——橙色小圆点，点击跳到专家页的**实时**运行时视图（见 [Backstage](./backstage)）
...
实时看每一个正在干活的数字专家——状态环头像、watchdog 卡死 / 孤儿判定、软停 / 强停 / 一键全清、按子 Agent 中断。有人说"Agent 卡住了"时打开的就是这一页。
...
HjjMate 用领域驱动的 Pinia store。每个 store **独占**自己的状态。
