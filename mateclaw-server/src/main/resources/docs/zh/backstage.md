---
title: Backstage —— Admin 运行时控制台，看见每个数字专家正在干什么
description: Backstage 是 admin 专用的实时视图，列出当前在岗的每一位数字专家。状态环头像、saying 行、watchdog 卡死 / 孤儿判定、软停 / 强停 / 一键全清、按子 Agent 中断。用户说"卡住了"的时候，你打开这一个 URL。
head:
  - - meta
    - name: keywords
      content: agent 运行时,运行时控制台,强制回收,卡死的 Agent,孤儿 run,多 Agent 可观测,SSE 流清理,数字专家,实时 Agent 可视
---
...
数字专家在某一步冻住，是 MateClaw 里少数不会自我修复的事情之一。流挂着、子 Agent 散到一地、SSE buffer 还活着但没人读。Backstage 是把这些东西全部摊开给你看、并且让你伸手干预的那块屏幕。
...
::: tip 1.4.0：实时视图并入专家页
v1.4.0 起，这块实时运行时视图被折叠进了**专家（Experts）页**。`/backstage` 现在会**重定向到** `/agents?view=live`；专家页顶部有一个 **花名册（Roster）/ 实时（Live）** 的分段切换——"实时" 就是本文描述的运行时控制台。下面提到的"路由 `/backstage`"仍然可用，只是会落到专家页的实时视图。
:::

- **路由：** `/backstage`（重定向到 `/agents?view=live`）
- **侧边栏：** 专家页的 **实时** 分段；当有专家卡死时，侧边栏会冒出一个**橙色"卡死专家"小圆点**，点它直接跳到这个实时视图
...
一栅卡片，每张卡片对应一位正在干活的数字专家。页面顶部那个自动刷新 chip 显示实时流是否在跑；点一下就暂停（比如你正要点按钮，不想卡片在你光标下挪位）。
