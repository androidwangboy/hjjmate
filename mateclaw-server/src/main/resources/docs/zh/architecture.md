...
**HjjMate 是怎么拼起来的，一页讲完。**

**用** HjjMate 的人看 [项目介绍](./intro)。**在 HjjMate 上面建东西**的人——加工具、新渠道、自定义记忆 provider、新的 Agent 图节点——看这一页。
...
**HjjMate 的 Agent 运行时不是一个类层次。** 没有 `BaseAgent` → `ReActAgent` → `MyCustomAgent` 的继承链。运行时是一张**由节点和条件边组成的 StateGraph**（来自 `spring-ai-alibaba-graph`），在运行时由 `AgentGraphBuilder` 装配。
...
- `agent/graph/state/HjjMateStateKeys.java`——共享 state 对象的 key
- `agent/graph/state/HjjMateStateAccessor.java`——state map 的类型化访问器（**别直接动 map**）
...
**加 Agent 行为**——在 `agent/graph/node/` 创建新节点，或在 `agent/graph/edge/` 创建新边 dispatcher。把它接进 `AgentGraphBuilder`。通过 `HjjMateStateAccessor` 读写 state。
...
- **渐进式工具/技能披露**——工具披露层把工具分成核心层（core）和扩展层（extension）两档；`enable_tool` / `load_skill` 让专家按需激活扩展工具、按需加载技能，从而把系统提示保持得足够小。
- **多级子专家委派树**——父专家到子专家的委派是递归的、有深度上限的，构成一棵树；子图的事件实时回流到根会话。
- **ChannelToolProvider SPI**——渠道（比如飞书）可以把平台能力直接作为专家工具暴露出来，不需要单独的 MCP 服务器。
...
更深的定制——在 `agent/graph/node/` 加新节点或在 `agent/graph/edge/` 加新 dispatcher。在配置 flag 后面接进 `AgentGraphBuilder`。State 访问走 `HjjMateStateAccessor`。

---

## 持久化 —— 一份 schema，两种数据库

HjjMate 用 **MyBatis Plus**（不是 JPA）做数据库访问。约定：
...
HjjMate 用 **Spring MVC**，不是 Spring WebFlux。**WebFlux 在依赖图里被明确排除。**
