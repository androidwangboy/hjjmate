...
description: HjjMate 作为 MCP 客户端，通过 Model Context Protocol 接入任意外部工具服务器。JSON-RPC 动态发现、SSE/stdio 双传输、与内置工具无缝统一。
head:
  - - meta
    - name: keywords
...
**MCP 是 HjjMate 跟"别人写的工具"对话的方式。**

Model Context Protocol 是 Anthropic 提出的一个开放标准，用来把 AI 模型和外部工具/数据连起来。一个 MCP 服务是一个进程——本地或远程——通过 JSON-RPC 向外宣告一组工具。HjjMate 扮演 MCP **客户端**：连接上去、通过 `tools/list` 发现工具、把它们当原生工具暴露给你的 Agent。**从 Agent 的视角，一个内置的 `@Tool` Spring bean 和一个从 MCP 服务来的工具，没有任何区别。**

这是 HjjMate 的**逃生口**。你需要一个 HjjMate 没自带的能力——沙盒目录的文件访问、Tavily 搜索、某个自定义的企业数据服务、一整套浏览器自动化——大概率已经有现成的 MCP 服务，你可以把它接进来，不用写一行 Java。
...
- **MCP 客户端**——HjjMate，负责连接 MCP 服务、发现工具、转发工具调用
- **MCP 服务**——第三方工具服务器
...
HjjMate 启动一个本地子进程，通过 stdin/stdout 交换 JSON-RPC 消息。

```
HjjMate  ── stdin ──►  MCP Server 子进程
          ◄─ stdout ──
```
...
HjjMate 支持把**认证用户名**注入到每次工具调用的参数里，让 MCP server 代表该用户调用底层 REST 后端。
...
开启后，HjjMate 在调用该 server 的每个工具时，往参数 JSON 里注入保留字段 **`__mateclaw_user__`**（值=认证用户名）。该值由受信服务端注入、**不经 LLM**；若 LLM 伪造了同名字段会被覆盖，因此模型无法冒充身份。无认证用户时不注入（不伪造身份）。
...
**② 签名 token（推荐用于跨信任边界）**：注入一个 HjjMate 用私钥现签的**短时 RS256 JWT**（保留字段换成 **`__mateclaw_token__`**），REST 后端用**公钥验签**——它信任的是签名，而非 MCP 服务/Python/传输。
...
生成密钥对（私钥配给 HjjMate，公钥配给 REST 后端）：
...
> `sub` 携带的是 HjjMate 用户标识（`ChatOrigin.requesterId`）。若后端按不可变数字 id 鉴权，可在签发前把用户名解析成 id（本层刻意不耦合用户存储）。
...
`@PreDestroy` 钩子正常会清理。HjjMate 被强杀（`kill -9`）的话子进程可能残留。`ps aux | grep mcp` 找到并杀掉。
