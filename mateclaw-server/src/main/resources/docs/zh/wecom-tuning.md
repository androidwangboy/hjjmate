...
[多渠道接入 → 企业微信](./channels#企业微信) 那一节是把 bot 跑起来；这一篇是把 bot **跑稳**——所有 HjjMate 在企业微信适配层做过的非显然优化、踩过的平台边角，以及为什么这么处理。
...
HjjMate 在 link 分支检测到 `mp.weixin.qq.com` 后，会自动给模型追加一段提示：
...
**解决**：在管理后台**取消勾选**全部 7 项数据权限，bot 即可对所有授权成员可见。HjjMate 通过 webhook 拿消息，不需要这些数据权限。
...
WeCom 群的 bot 必须被 `@` 才收到消息。私聊不需要 `@`。这是平台行为，没办法绕过。HjjMate 不会在群里 broadcast 监听所有消息（也做不到）。
