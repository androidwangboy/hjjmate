...
这是唯一一件需要记住的事。HjjMate 是 Apache 2.0、自部署、一个 JAR 包发布的产品。你加的每一行都会被某个你永远不会见到的人下载、解压、执行。**写那种六个月之后你在别人家的日志里凌晨两点看到也会开心的代码。**
...
- **一个 state key**——在 `vip.mate.agent.graph.state.HjjMateStateKeys`
