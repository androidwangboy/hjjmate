---
title: 持久化目标 — 跨多轮锁定，让专家自己跟进
description: HjjMate 的 Goal 系统让数字专家把跨多轮的任务锁成一个目标，自己评估进度、自己续命，直到完成或耗尽预算。
head:
  - - meta
    - name: keywords
      content: Goal,目标管理,Agent,多轮对话,自动评估,auto-followup,持久化,HjjMate
---

# 持久化目标

> **以前你每轮都要把上下文重复一遍。现在你定一个目标，专家自己跟。**

一次对话里你说"帮我把这个博客部署到 fly.io"，专家答完一轮就停了。下一轮你要再问"DNS 配好没？证书呢？测试跑了吗？"——你在替它记目标。

Goal 把这件事翻过来。**你说一次，专家锁住目标，自己每轮自检：还差什么？要不要自己再做一步？**

它不是聊天里的一个新功能。它是专家的一种**状态**。专家头像周围多了一圈光，光填多少就是离完成多远。完成了，光消失。
...
你只要在第一次描述任务时让专家知道这是个长任务：

> 我要做一个完整的项目：把 README 翻译成英文、提 PR、走 review、合并。这跨多轮，**请你用 setGoal 锁定**，每轮自我评估，turnBudget=8，autoFollowup 开启。

专家识别到"长任务"+"明确要求 setGoal"两条信号，会自动调用工具创建目标，title 从对话上下文自动归纳。你只需点开它的回答，看见头像旁边多了一圈光，就知道目标已锁。

### 方式 2 — 直接命令工具

不想让专家判断，你直接告诉它调哪个工具、传什么参数：

> 请立刻调用 setGoal 工具，title="部署博客到 fly.io"，turnBudget=10，autoFollowup=true。不要问任何前置确认。

"不要问前置确认"这一句很重要 — 否则专家会先问"代码在哪？域名是什么？" 它的本能就是先澄清。
...
- **自动延续 (autoFollowupEnabled)**：开了之后，专家答完一轮如果觉得"还没完成"，会自己接着做下一步，不等你催
...
每次专家回答完一轮，后台会跑一个评估节点。这个节点：

1. 取专家这一轮的最终回答 + 最近几条消息上下文
...
3. 让专家再跑一轮 reasoning，**这一轮的回答就直接接在第一轮后面**

你的体感是：专家答完一段 → 停半拍 → **继续往下做** — 就像一个人做完一步停了一下想了想然后继续。
...
::: tip 新增
长任务最怕的不是难，是**卡住**——撞到迭代上限就停、某一步失败就崩、在一个工具上空转到预算耗尽。这一组机制让专家在这些地方能自己续上、绕开、爬起来，而不是停在那等你再发消息。
:::

### 撞到迭代上限的硬延续

以前一轮 ReAct 跑满 `max_iterations`（结束原因 `MAX_ITERATIONS_REACHED`），目标子系统会**跳过**这一轮——不评估、不延续，任务就停在那。现在它走一条**硬延续**路径：把迭代计数清零、清掉"超限草稿"，给专家**一段全新的满额迭代预算**接着干。
...
> 1.4.0 的目标是"专家记住它在干什么"。1.5.0 的目标是"专家知道**具体还差哪几条**"。从一个分数，到一份能逐条勾的清单。

---

## 4 个内置工具（专家可用）

专家的工具集里默认包含这 4 个（无需手动绑定，是 agent-wide 系统级工具）：
...
完成时（`completeGoal`，或 evaluator 判定**每一条准则都通过**），专家会把这个目标的总结同步到[长期记忆](./memory)，后续对话能查得回来。

---

## 子专家不能改父专家的目标

[多专家协作](./agents)里 parent 专家可以委派 child 专家干活。Child **看不到**这 4 个 goal 工具 — 目标是 parent 会话的状态，child 是无状态的执行体。
...
任一条命中 → 目标状态翻为 **exhausted**，不再触发评估、不再注入 follow-up，光环变橙红色。专家的最后一轮回答会正常发送给你。
...
**Goal 不是给专家加一个功能。是改它的状态。**

以前的专家"答完就忘"。Goal 让专家跨多轮记住一件事 — 它在干什么、还差什么、什么时候算完。你只用说一次。剩下的，让头像旁边那圈光替你跟。
