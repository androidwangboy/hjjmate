...
HjjMate 的 WebChat 渠道让外部网站通过纯 HTTP / SSE 接入对话能力,无需 JWT。访客身份通过 `visitorId + visitorToken`(HMAC 签发)在共享的 API Key 之下做隔离。
...
<script>
  HjjMateWebChat.init({
    ...
    server: 'https://<你的部署地址>',  // HjjMate 服务地址
    title: '在线客服',
    placeholder: '输入消息...'
  })
</script>
...
- **Base URL**:`https://<你的 HjjMate 部署地址>/api/v1/channels/webchat`
...
管理员级(需要 HjjMate JWT,不在本表的 permitAll 范围内):
