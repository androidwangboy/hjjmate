...
HjjMate can expose enabled agents through an A2A JSON-RPC endpoint and can call other A2A peers from agent tools.
...
`base-url` is required for production deployments. If it is empty, HjjMate derives the Agent Card URL from the incoming request, which depends on proxy headers being correct.
...
- `POST /api/a2a` requires an existing HjjMate Bearer token and supports `message/send`, `message/stream`, `tasks/get`, and `tasks/cancel`.
