...
Chat is where you actually work. Everything else in HjjMate — agents, tools, memory, wiki, channels — exists so that what happens inside this box can be good.
...
One of the questions HjjMate tries to answer with its chat UI is: **should you trust what the AI just told you?** The default answer elsewhere is "look at the answer and guess". HjjMate tries to do better.
...
Trust is earned by showing the work. HjjMate shows the work.
...
Drop a **folder** on the desktop app and the agent gets a reference to the folder's absolute path — it can then walk it with the file-reader or shell tool. Drop a folder on the web and HjjMate recursively expands it and uploads each file individually.
...
A conversation is a sequence of messages scoped to a single agent and a single user. HjjMate stores them in two tables:
...
Every turn, HjjMate builds the prompt that actually goes to the LLM. Roughly:
