...
description: HjjMate acts as an MCP client, connecting to any external tool server via Model Context Protocol. JSON-RPC dynamic discovery, SSE/stdio dual transport, seamless unification with built-in tools.
head:
  - - meta
    - name: keywords
...
**MCP is how HjjMate talks to tools someone else built.**

Model Context Protocol is an open standard from Anthropic for connecting AI models to external tools and data. An MCP server is a process — local or remote — that advertises a set of tools over JSON-RPC. HjjMate acts as an MCP *client*: it connects, discovers tools via `tools/list`, and exposes them to your agents as if they were native. **From the agent's point of view, there's no difference between a built-in `@Tool` Spring bean and a tool coming from an MCP server.**

This is the escape hatch. If you need a capability HjjMate doesn't ship with — filesystem access for a sandboxed directory, Tavily search, a custom internal data service, a browser automation suite — there's probably already an MCP server for it, and you can plug it in without writing a line of Java.
...
- **MCP Client** — HjjMate, connecting to servers, discovering tools, forwarding invocations
...
HjjMate spawns a local child process and exchanges JSON-RPC messages via stdin/stdout.

```
HjjMate  ── stdin ──►  MCP Server subprocess
          ◄─ stdout ──
```
...
```
HjjMate  ── HTTP POST ──►  Remote MCP Server
          ◄─ HTTP Stream ──
```
...
| Process management | HjjMate manages subprocess | External | External |
...
Save. If enabled, HjjMate auto-attempts to connect and discover tools.
...
HjjMate can inject the **authenticated username** into every tool call for a
...
When enabled, HjjMate injects the reserved argument **`__mateclaw_user__`**
...
**RS256 JWT** that HjjMate signs with a private key (reserved key becomes
...
Generate the key pair (private → HjjMate, public → REST backend):
...
> `sub` carries the HjjMate user identifier (`ChatOrigin.requesterId`). If your
...
1. Confirm the command is in PATH of the user running HjjMate
...
Subprocesses are cleaned up on normal shutdown. If HjjMate was force-killed (`kill -9`), subprocesses may remain. `ps aux | grep mcp` and terminate.
