...
**How HjjMate is put together, in one page.**

If you're using HjjMate, read [Introduction](./intro). If you're building on HjjMate — adding tools, new channels, custom memory providers, new agent graph nodes — read this page.
...
**HjjMate's agent runtime is not a class hierarchy.** There's no `BaseAgent` → `ReActAgent` → `MyCustomAgent` inheritance chain. The runtime is a **StateGraph** (from `spring-ai-alibaba-graph`) composed of nodes and conditional edges, assembled at runtime by `AgentGraphBuilder`.
...
- `agent/graph/state/HjjMateStateKeys.java` — the keys for the shared state object
- `agent/graph/state/HjjMateStateAccessor.java` — typed accessor for the state map (don't touch the map directly)
...
**Adding agent behavior** — create a new node in `agent/graph/node/` or a new edge dispatcher in `agent/graph/edge/`. Wire it into `AgentGraphBuilder`. Read and write state through `HjjMateStateAccessor`.
...
For deeper customization, add a new node in `agent/graph/node/` or a new dispatcher in `agent/graph/edge/`. Wire it into `AgentGraphBuilder` behind a config flag. State access goes through `HjjMateStateAccessor`.

---

## Persistence — one schema, two databases

HjjMate uses **MyBatis Plus** (not JPA) for database access. Conventions:
...
HjjMate uses **Spring MVC**, not Spring WebFlux. WebFlux is explicitly excluded from the dependency graph.
