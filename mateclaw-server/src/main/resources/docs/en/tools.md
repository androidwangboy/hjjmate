...
**1. Built-in tools.** The twenty tools that ship with HjjMate — registered into the tool table on startup.

**2. MCP servers.** External processes speaking the Model Context Protocol expose tools dynamically. HjjMate discovers them via `tools/list` and they appear in the registry alongside built-in ones. See [MCP](./mcp).
...
| `HjjMateDocTool` | Read built-in project documentation | — |
...
Reads the built-in HjjMate project documentation. Lets an agent answer "how does X work in HjjMate" questions by consulting actual docs rather than guessing.
...
Tool Guard is how HjjMate keeps strong tools from doing stupid things. It's **rule-based**, not a flat dangerous-tools list. Each rule says: *for this tool, with these arguments, in this context, do X* — where X is `allow`, `deny`, or `require_approval`.
...
With a large tool catalog, HjjMate exposes a lightweight directory first and expands concrete schemas only when the task needs them. The progressive bridge reduces context pressure; normalized, enabled tool names are cached so long loops do not repeatedly scan the MCP hot path.
