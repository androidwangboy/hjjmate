...
The [Channels → WeCom](./channels#wecom) section covers wiring up the channel; this document covers what HjjMate does **after** the channel is up — every non-obvious optimization, every platform corner the adapter handles, and why.
...
```
[@XuZhanFu] @HjjMateBot I want to query X
[@xuzf] @HjjMateBot I want to query Y
```
...
When HjjMate detects `mp.weixin.qq.com` in the link branch, it appends a directive to the model:
...
**Fix**: in the admin panel, **uncheck** all 7 data permissions. The bot becomes available to all authorized members. HjjMate uses webhooks for messages, doesn't need data permissions.
...
The bot in a WeCom group must be `@`-mentioned to receive a message. Direct messages (1:1) don't need `@`. Platform behavior, no workaround. HjjMate doesn't broadcast-listen to all group messages (and couldn't if it tried).
