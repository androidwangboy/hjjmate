...
**Java 17 or higher.** HjjMate uses features introduced in Java 17 (sealed classes, text blocks, records, pattern matching). Verify with `java -version`.
...
- **Ollama** — local GPU inference; HjjMate auto-detects it on `localhost:11434` at startup
...
**You also don't need to set any API key as an environment variable to start HjjMate.** All provider configuration is done through the UI at `Settings → Models` after startup.
...
2. Sign up or log in
3. Create an API key
4. In HjjMate, go to `Settings → Models → DashScope` and paste it
...
HjjMate implements **sliding-window renewal** — when a token is within 25% of expiry, the server issues a new one in the `X-New-Token` response header. The frontend handles this automatically.
...
### How do I use GPT-4 with HjjMate?
...
- macOS: `~/Library/Application Support/HjjMate/`
- Windows: `%APPDATA%/HjjMate/`
- Linux: `~/.local/share/HjjMate/`
...
- macOS: `~/Library/Logs/HjjMate/`
- Windows: `%APPDATA%/HjjMate/logs/`
- Linux: `~/.local/share/HjjMate/logs/`
