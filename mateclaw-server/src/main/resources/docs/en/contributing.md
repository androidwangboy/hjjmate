...
That's the only thing to remember. HjjMate is Apache 2.0, self-hosted, and shipped as a single JAR. Every line you add gets downloaded, unpacked, and executed by someone you'll never meet. Write the code you'd be happy to find yourself, six months from now, in someone else's logs at 2 AM.
...
- **A state key** — in `vip.mate.agent.graph.state.HjjMateStateKeys`
...
- HjjMate version (or commit hash)
- Java version and OS
