...
- **Windows** — `HjjMate-Setup-x.y.z.exe`
- **macOS** — `HjjMate-x.y.z.dmg`
- **Linux** — `HjjMate-x.y.z.AppImage`
...
- **Ollama** — local GPU users; HjjMate auto-detects `localhost:11434`
...
- **Backend never boots** — Check the log file (macOS: `~/Library/Application Support/HjjMate/logs/mateclaw.log`; Windows: `%APPDATA%\HjjMate\logs\mateclaw.log`). The desktop app picks a dynamic port — any port conflict is reported clearly in the log.
