...
|------|--------------------|------------------|
| **`builtin`** | Ships with HjjMate under `skills/` in the classpath | The core team |
...
- **Built-in** — skills that ship with HjjMate
...
A skill that's installed isn't necessarily a skill that runs — it might need an API key, a CLI tool, a HjjMate feature flag toggled on.
...
ACP (Agent Client Protocol) is a protocol that lets external agent clients (Claude Code, Codex, other compatible clients) plug into HjjMate as skills.
