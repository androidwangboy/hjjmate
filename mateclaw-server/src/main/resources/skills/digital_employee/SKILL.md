...
nameZh: 数字专家组建
nameEn: Digital Expert Builder
version: "1.0.0"
icon: "🧑‍💼"
description: "根据一句业务需求，自动规划并创建一组分工明确的数字专家（Agent），再把他们编排成一条工作流，无需逐个手工创建。"
...
把一句业务需求，变成「一支分工明确的数字专家团队 + 一条把他们串起来的工作流」。用户不必逐个设计、逐个手工创建 Agent。

## 何时使用

- 用户描述了一个**需要多个角色协作**的目标（"帮我搭一个做竞品分析的团队"、"我要一套从线索到成交的销售流程"）。
- 用户说"建几个 Agent / 数字专家帮我做 X"、"把 X 这件事自动化成一支团队"。
...
- 只是想把流程做成工作流，且专家都已存在 → 直接用 `workflow_draft_generate`。
...
2. 调用 `listAvailableAgents()` 看现有专家——**能复用就复用**，不要重复造同名角色。
...
- `systemPrompt`：定义这个专家的专长、视角、工作方式——要具体，别写空话。
...
- 工具会返回 `agentId` 和实际绑定的技能/工具；**记下每个专家的 name**，下一步要用。
- 若返回 `[error]`（如重名），换个名字重试，不要中断整个流程。
- 创建即启用——专家立刻可被工作流引用。

### 第四步：编排工作流

所有专家创建完成后，调用一次 `workflow_draft_generate(description=...)`。在 `description` 里：

- **点名第三步创建的真实专家**（用它们的 name），说明执行顺序、依赖关系、并行还是串行。
- 写清触发条件、是否需要审批、产出发往哪个渠道。

由于专家已经落库，`workflow_draft_generate` 会读到它们作为可用数字专家，直接引用真实 agent，而不是填 `TODO_*_AGENT` 占位。
...
- 专家 name 用英文 kebab-case 且工作区内唯一；工作流 step.name / outputVar 的命名约束由 `workflow_draft_generate` 负责，本技能不必操心。
- 技能名 / 工具名必须来自 `list_capability_catalog()`，臆造的名字会被静默跳过。
- 先创建专家，**再**生成工作流——顺序不能反，否则工作流只能拿到占位符。
